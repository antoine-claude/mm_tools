...
class KITSU_build_shot_candidate(bpy.types.PropertyGroup):
    """Represents an asset candidate for build_shot linking.
    
    Automatically populated by the Kitsu cache system when the active shot changes.
    The dynamic BoolProperties (buildshot_*) are created in kitsu.cache for user
    selection, and their names are stored here for reference in the UI.
    """
    # name: StringProperty() -> Instantiated by default (asset name)
    pass
...
class KITSU_property_group_scene(bpy.types.PropertyGroup):
    ...
    build_shot_candidate_names: bpy.props.CollectionProperty(
        type=KITSU_build_shot_candidate,
        name="Build Shot Candidates",
        description="Asset candidates available for the active shot (managed by Kitsu cache system)"
    )
