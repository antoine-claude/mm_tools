...
class KITSU_OT_con_productions_load(bpy.types.Operator):
    """
    Gets all productions that are available on server and lets user select.
    Invokes a search popup (enum_prop) on click.
    """
...
def auto_login_on_file_open():
    """Auto-login if credentials are configured in preferences."""
    try:
        context = bpy.context
        session = prefs.session_get(context)
        addon_prefs = prefs.addon_prefs_get(context)
        ...
        if addon_prefs.email and addon_prefs.passwd and addon_prefs.host:
