...
def load_wheels_global_together(main_module: str, wheels_prefixes: list[str]):
    """
    Charge un module (ex: 'openpyxl') en exposant plusieurs wheels en même temps 
    dans sys.path (ex: ['openpyxl', 'et_xmlfile']) et RETIENT le module dans 
    sys.modules pour que les autres fichiers puissent l'importer.
    """

    wheel_paths = [str(_wheel_filename(prefix)) for prefix in wheels_prefixes]

    # Ajouter les wheels EN PERMANENCE dans sys.path
    ...
    # Import principal (reste dans sys.modules)
    try:
        module = importlib.import_module(main_module)
    except Exception as ex:
        raise ImportError(
            f"Unable to load {main_module} using wheels {wheel_paths}: {ex}"
        ) from ex

    _log.info("Loaded %s from %s", main_module, module.__file__)
    return module